...
if (isset ($_POST['submitted_search'])) {
    ...
    $tag = $tagMapper->getTagById((int)$_GET['tag']);
    if ($tag) {
        $this->view->vars->cleaned = $tag->tag;
        $this->view->vars->tagId = $tag->tagId;
    } else {
        $this->view->vars->cleaned = Language::getText('invalid_tag');
    }
}
...
if ($tag_search) {
    // tag search
    if (isset($this->view->vars->tagId)) {
        $query_string['tag'] = $this->view->vars->tagId;
        ...
        $this->view->vars->search_videos = $tagService->getVideosByTagId($this->view->vars->tagId, $start_record, $records_per_page);
    }
